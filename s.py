    ...
    while True:
        try:
            ...
            if received_hash == calculated_hash:
                print(f"Integrity verified: Message from {client_name} is not tampered.")
                if recipient in clients:
                    # Forward the message to the recipient
                    recipient_socket = clients[recipient]
                    recipient_socket.send(f"\n Message recieved from {client_name}: {content}".encode())
                else:
                    client_socket.send("Recipient not found or offline".encode())
            else:
                print(f"Integrity compromised: Message from {client_name} may have been tampered.")
...
def main():
    ...
    while True:
        client_socket, address = server_socket.accept()
        print(f"Connection established with {address} and cleint socket {client_socket}")
        threading.Thread(target=handle_client, args=(client_socket, address)).start()
if __name__ == "__main__":
    main()
